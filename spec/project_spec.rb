...
RSpec.describe 'RuboCop Performance Project', type: :feature do
  describe 'department cop registration' do
    it 'registers every cop file in `lib/rubocop/cop/performance` exactly once' do
      cop_root = File.expand_path('../lib/rubocop/cop', __dir__)
      files = Dir[File.join(cop_root, 'performance', '*.rb')].sort

      registered = RuboCop::Cop::Registry.global.cops_for_department(:Performance).map do |cop|
