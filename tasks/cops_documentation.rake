...
YARD::Rake::YardocTask.new(:yard_for_generate_documentation) do |task|
  task.files = ['lib/rubocop/cop/performance/*.rb']
...
task generate_cops_documentation: :yard_for_generate_documentation do
  ...
  def print_cops(cops, config)
    content = "# Performance\n".dup
    cops.each do |cop|
      content << print_cop_with_doc(cop, config)
    end
    file_name = "#{Dir.pwd}/manual/cops_performance.md"
  ...
  def table_of_contents(cops)
    filename = 'cops_performance.md'

    cops.to_enum.map do |cop|
      anchor = cop.cop_name.sub('/', '').downcase
      "* [#{cop.cop_name}](#{filename}##{anchor})"
    end.join("\n")
  end

  def print_table_of_contents(cops)
    path = "#{Dir.pwd}/manual/cops.md"
    File.write(path, table_of_contents(cops))
  end
  ...
  def main
    cops = RuboCop::Cop::Cop.registry.with_department(:Performance).sort!
    config = RuboCop::ConfigLoader.default_configuration

    YARD::Registry.load!

    print_cops(cops, config)
